...
function johanna_filter (shoesArray, x, y) {

    let newShoesArray = [];

    for (i = 0; i < shoesArray.length; i++) {
        
        let shoePrice = shoesArray[i].price;

        if (shoePrice >= x && shoePrice <= y) {
            newShoesArray.push(shoePrice);
        }
    }
    return newShoesArray;
}

johanna_filter (SHOES, 200, 800);